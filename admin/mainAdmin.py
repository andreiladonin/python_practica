...
from admin.menuAdmin import Ui_MenuAdnin
from admin.adminka import adminka
from admin.deleteCompany import windowDellCompany
from admin.appendCompany import *
from admin.windowAppendContract import windowAppendContract
from admin.DelleteContractWindow import DelleteContractWindow
from admin.windowAddOrChangeAct import windowDellOrChangedAct
from admin.windowChangedAct import windowChangedAct
from admin.actDelete import windowDellAct
# окно админа
...
def adminMenu():
    ...
    # ф-ция обновление виджета таблицы из бд
    ...
    # ф-ия для удалений компаний
    def loadWindowDellCompany():
        x = windowDellCompany()
        if x == 0:
            updateTableFirms()
            updateTableContracts()
    ...
    def changedTableContracts():
        ...
        if bool(tbl):
            ...
            db.commit()
            for i in tbl:
                cursor.execute(f"INSERT or IGNORE INTO contracts (idCompany, nameCompany, workCompany, summaContract) VALUES ({int(i[1])}, '{i[2]}','{i[3]}', '{i[4]}')")
                db.commit()
                cursor.execute(f"UPDATE contracts SET nameCompany = '{i[2]}' WHERE idCompany = {int(i[1])}")
                db.commit()
            msg = QMessageBox()
            msg.setWindowTitle("Успешно")
            msg.setIcon(QMessageBox.Information)
            ...
            updateTableContracts()
            updateTableContracts()
    # ф-ия для  УДАЛЕНИЯ ДОГОВОРОВ
    def loadDelleteContractWindow():
        x = DelleteContractWindow()
        if x == 0:
            updateTableContracts()
            updateTableAct()
    ...
    # ф-ции для АКТОВ
    # ф-ция для добавл./изменения актов
    def loadWindowAct():
    ...
    ui.delDirectory.clicked.connect(loadWindowDellCompany)
    ui.changeDirectory.clicked.connect(changedTblCompany)
    ui.buttAdmin.clicked.connect(adminka)
    ...
    ui.delContact.clicked.connect(loadDelleteContractWindow)
    ...
    ui.delAct.clicked.connect(loadWindowDellAct)
